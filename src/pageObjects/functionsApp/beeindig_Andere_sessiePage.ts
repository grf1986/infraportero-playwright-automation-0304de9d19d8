...
import { Beëindig_andere_sessie } from "../../functions/beëindig_andere_sessie";
import { NavigeerNaarTab } from "../../functions/navigeerNaarTab";


export class beeindig_Andere_sessiePage {
  readonly page: Page;
  ...
  async Beëindig_andere_sessie(tab:number) {
         const targetTab = await new NavigeerNaarTab(this.page).navigeerNaarTab(tab);
            if (targetTab){
              await new Beëindig_andere_sessie(targetTab).Beëindig_sessie()
                            }
            else { return }
}
